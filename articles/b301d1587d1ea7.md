...
:::message
VRT でロジックを見ようとすると地獄になります。ロジックの正しさは別のテストで担保し、VRT は「意図しない見た目の破壊」だけを検出するようにしましょう。
:::
...
ここまで読んで、「じゃあステータスの種類が正しいかはどうやって確認するの？」と思った方もいるかもしれません。

答えは、**RTL（React Testing Library）でロジックをテストする**ことです。

## ロジックと見た目の責務分離

- **RTL**: ステータスが正しく切り替わるか？（ロジックテスト）
- **VRT**: ステータス表示がレイアウト崩壊しないか？（見た目テスト）

例えば、RTL では以下のようなテストを書きます：
...
このように、**ロジックは RTL で全種類をカバー**し、**見た目は VRT で代表例だけをカバー**するのが理想的です。
