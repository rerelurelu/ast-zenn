...
答えは、**テストライブラリ（RTL など）を使ってコンポーネント単位でロジックをテストする**ことです。

## ロジックと見た目の責務分離

- **ロジックテスト**: ステータスが正しく切り替わるか？（全パターンをテスト）
- **VRT**: ステータス表示がレイアウト崩壊しないか？（代表例のみテスト）

例えば、React Testing Library では以下のようなテストを書きます：
