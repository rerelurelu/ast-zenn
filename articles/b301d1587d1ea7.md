...
- **バリエーションの代表例**: `primary`/`secondary`/`danger` のうち 1 つ（色違いは構造が同じなので代表だけ）
- **アイコンの有無**: アイコン付き/なしを各 1 つずつ（構造が異なる）
- **合計**: 1×2 = 2 パターン

これで、ボタンのレイアウトが崩れていないか、アイコンの位置がずれていないかを確認できます。色違いの `secondary` や `danger` は、`primary` と構造が同じなので VRT では省略できます。
...
このテーブルには、ステータス列が 3 種類あり、それぞれ振る舞いが異なります。
...
このテーブルには 3 種類のステータスがあり、それぞれ異なる振る舞いをします：

- `completed`と`error`はアイコン付き
- `pending`はアイコンなし
- それぞれ色も違う

「全部のパターンが違うから、3 種類すべてを VRT しなきゃ！」と思いたくなりますが、本当にそうでしょうか？
